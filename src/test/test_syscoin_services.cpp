...
bool AssetFilter(const string& node, const string& cert)
{
	UniValue r;
	int64_t currentTime = GetTime();
	string query = "\"{\\\"_id\\\":\\\"" + cert + "\\\"}\"";
	BOOST_CHECK_NO_THROW(r = CallRPC(node, "syscoinquery asset " + query));
	const UniValue &arr = r.get_array();
	return !arr.empty();
}
bool AssetAllocationFilter(const string& node, const string& cert)
{
	UniValue r;
	int64_t currentTime = GetTime();
	string query = "\"{\\\"_id\\\":\\\"" + cert + "\\\"}\"";
	BOOST_CHECK_NO_THROW(r = CallRPC(node, "syscoinquery assetallocatiion " + query));
	const UniValue &arr = r.get_array();
	return !arr.empty();
}
...
void AssetNew(const string& node, const string& name, const string& alias, const string& pubdata, const string& supply, const string& maxsupply, const string& useinputranges, const string& interestrate, const string& canadjustinterest, const string& witness)
{
	string otherNode1, otherNode2;
	GetOtherNodes(node, otherNode1, otherNode2);
	UniValue r;
	...
	BOOST_CHECK_EQUAL(find_value(historyResultObj, "type").get_str(), "Asset Activated");
	...
	BOOST_CHECK(find_value(r.get_obj(), "balance").get_int64() == boost::lexical_cast<int64_t>(supply));
	BOOST_CHECK(find_value(r.get_obj(), "total_supply").get_int64() == boost::lexical_cast<int64_t>(supply));
	BOOST_CHECK(find_value(r.get_obj(), "max_supply").get_int64() == boost::lexical_cast<int64_t>(maxsupply));
...
void AssetUpdate(const string& node, const string& name, const string& pubdata, const string& supply, const string& interest, const string& witness)
{
	...
	CAmount oldsupplyamount = find_value(r.get_obj(), "total_supply").get_int64();
	CAmount supplyamount = 0;
	if(supply != "''")
		supplyamount = ValueFromString(supply).get_int64();
	...
	BOOST_CHECK_EQUAL(find_value(historyResultObj, "type").get_str(), "Asset Updated");
	...
	BOOST_CHECK_EQUAL(find_value(r.get_obj(), "total_supply").get_int64(), newamount);
	GenerateBlocks(5, node);
	if (!otherNode1.empty())
	{
		BOOST_CHECK_NO_THROW(r = CallRPC(otherNode1, "assetinfo " + name + " false"));
...
const string OfferLink(const string& node, const string& alias, const string& guid, const string& commissionStr, const string& newdetails, const string &witness)
{
	...
	const UniValue &historyResultObj = ret.get_obj();
	BOOST_CHECK_EQUAL(find_value(historyResultObj, "user1").get_str(), alias);
	BOOST_CHECK_EQUAL(find_value(historyResultObj, "_id").get_str(), txid + "-" + linkedguid);
...
void OfferUpdate(const string& node, const string& aliasname, const string& offerguid, const string& category, const string& title, const string& qtyStr, const string& price, const string& description, const string& currency, const string &isprivateStr, const string& certguid, const string& commissionStr, const string& paymentoptions, const string& offerType, const string& auction_expires, const string& auction_reserve, const string& auction_require_witness, const string &auction_deposit, const string& witness) {
	...
	const UniValue &txHistoryResult = AliasTxHistoryFilter(node, txid + "-" + offerguid);
	BOOST_CHECK(!txHistoryResult.empty());
	UniValue ret;
	BOOST_CHECK(ret.read(txHistoryResult[0].get_str()));
	const UniValue &historyResultObj = ret.get_obj();
...
void EscrowFeedback(const string& node, const string& userfrom, const string& escrowguid, const string& feedback, const string& rating,  const string& userto, const string& witness) {
	...
	const UniValue &txHistoryResult = AliasTxHistoryFilter(node, txid + "-" + escrowguid);
	BOOST_CHECK(!txHistoryResult.empty());
	UniValue ret1;
